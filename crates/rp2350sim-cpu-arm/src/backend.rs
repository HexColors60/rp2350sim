//! ARM Cortex-M33 backend.

use crate::state::ArmState;
use crate::thumb::decode;
use crate::thumb::execute::{execute, MemoryAccess};
use rp2350sim_core::{CoreId, CpuStepResult, Result};
use rp2350sim_cpu_common::CpuBackend;

/// ARM Cortex-M33 CPU backend.
...
impl ArmBackend {
    ...
    pub fn state_mut(&mut self) -> &mut ArmState {
        &mut self.state
    }
    ...
    pub fn flags(&self) -> u32 {
        self.state.core(CoreId::new(0)).map(|c| c.xpsr.bits()).unwrap_or(0)
    }
    ...
    pub fn step_with_memory(&mut self, memory: &mut impl MemoryAccess) -> Result<CpuStepResult> {
        ...
        // Fetch instruction at PC (Thumb mode - 16-bit instructions)
        let pc = core.pc;
        let opcode = memory.read_half(pc)?;
...
impl CpuBackend for ArmBackend {
    fn reset(&mut self) {
        self.state.reset();
    }
    ...
    fn set_irq(&mut self, line: usize, level: bool) {
        self.state.nvic.set_irq(line, level);
    }
